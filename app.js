...
function renderPlayers() {
    ...
    if (tbody) tbody.innerHTML = state.players.map((p, i) => `
        <tr style="${p.dropped ? 'opacity: 0.5; background: #eee;' : ''}">
            <td>${i+1}</td>
            <td>${p.name} ${p.dropped ? '(DROPPED)' : ''}</td>
            <td>${p.faction}</td>
            <td>
                ${isAdmin ? `
                    <button class="btn btn-danger btn-sm" onclick="removePlayer('${p.id}')">Delete</button>
                    <button class="btn btn-warning btn-sm" onclick="toggleDropPlayer('${p.id}', ${p.dropped || false})">
...
function renderTableMonitor() {
    ...
    if (!activeRound || !activeRound.pairings) return monitor.innerHTML = "<p>No active round pairings found.</p>";
...
function renderPending() {
    ...
    if (state.pending.length === 0) return list.innerHTML = "<p>No registration requests.</p>";
...
function renderPairings() {
    ...
    const activeRound = state.rounds.find(r => r.id === state.activeRoundId);
    if (!activeRound) return container.innerHTML = "<p class='card'>No active round created.</p>";
    ...
    container.innerHTML = `<h3>${activeRound.label}</h3>` + toShow.map(m => {
        ...
        const scoreFields = (side) => `
            <div class="form-grid">
                ...
                <input type="number" class="score-input" data-mid="${m.id}" data-side="${side}" data-type="paint" value="${r[side]?.paint || 10}">
            </div>`;
...
document.getElementById("btnAddPlayer").onclick = () => {
    ...
    .then(() => { nameEl.value = ""; factEl.value = ""; });
};

window.toggleDropPlayer = (id, currentStatus) => {
    if (!isAdmin) return;
    update(ref(db, `tournament/players/${id}`), { dropped: !currentStatus });
};
...
window.approvePlayer = (uid) => {
    ...
    const updates = {};
    updates[`tournament/players/${uid}`] = { ...p, dropped: false };
...
document.getElementById("btnNextRound").onclick = () => {
    ...
    update(ref(db), { [`tournament/rounds/${idx}`]: { id, label: `Round ${idx+1}`, pairings: {} }, 'tournament/activeRoundId': id });
};

document.getElementById("btnGeneratePairings").onclick = () => {
    if (!isAdmin) return;
    const rIdx = state.rounds.findIndex(r => r.id === state.activeRoundId);
    if (rIdx === -1) return alert("Create a round first!");
    
    // Only pair players who are NOT dropped
    let pool = state.rounds.length === 1 
        ? state.players.filter(p => !p.dropped).sort(() => 0.5 - Math.random()) 
        : computeStandings().filter(p => !p.dropped);
...
window.removePlayer = (id) => isAdmin && confirm("Delete entirely?") && remove(ref(db, `tournament/players/${id}`));
